...
using Cognite.MembershipProvider.Resources;
using Cognite.MembershipProvider.Common;
...
namespace Cognite.MembershipProvider
{
    public class SimpleMembershipProvider : ExtendedMembershipProvider
    {
        ...
        private readonly System.Web.Security.MembershipProvider _previousProvider;
        ...
        public SimpleMembershipProvider(System.Web.Security.MembershipProvider previousProvider)
        {
        ...
        private System.Web.Security.MembershipProvider PreviousProvider
        {
            get
            {
                if (_previousProvider == null)
        ...
        private int VerifyUserNameHasConfirmedAccount(IDatabase db, string username, bool throwException)
        {
            ...
            long result = db.QueryValue(@"SELECT COUNT(*) FROM " + MembershipTableName + " WHERE (UserId = @0 AND IsConfirmed = 1)", userId);
            if (result == 0)
            {
                if (throwException)
                {
